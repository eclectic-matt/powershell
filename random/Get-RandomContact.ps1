...
. C:\Users\matthew.tiernan\Desktop\POWERSHELL\utilities\random\Get-RandomName.ps1 > $silent;
. C:\Users\matthew.tiernan\Desktop\POWERSHELL\utilities\random\Get-RandomNumber.ps1 > $silent;

#Cls

function Get-RandomContact($name){
    #INIT CONTACT ARRAY
    ...
    $landline = Get-RandomNumber -intro $landlineIntro -length $numberLength;
    ...
    $mobile = Get-RandomNumber -intro $mobilesIntro -length $numberLength;
    ...
    $work = Get-RandomNumber -intro $worksIntro -length $numberLength;
    $contact.work = $work;
