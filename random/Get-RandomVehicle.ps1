...
function Get-RandomVehicle(){
    ...
    #ENGINE
    $engine = Get-RandomNumber -intro "1"  -length 4;
    #Write-Output ("ENGINE: " + $engine);

    #SERIAL (VIN)
    $serial = Get-RandomString -chars "ABCDEFGHJKLMNPRSTUVWXYZ0123456789" -length 17;

    #MILEAGE
    $mileage = Get-RandomNumber -intro "" -length 5;

    #PURCHASE PRICE
    $purchasePrice = Get-RandomNumber -intro "" -length 5;

    #REGISTRATION
    $regRegion = Get-RandomString -chars "ABCDEFGHJKLMNPRSTUVWXYZ" -length 2;
    $regYear = Get-RandomNumber -intro "" -length 2;
    $regSuffix = Get-RandomString -chars "ABCDEFGHJKLMNPRSTUVWXYZ" -length 3;
    $registration = ($regRegion + $regYear + $regSuffix);
    #Write-Output ("REGISTRATION: " + $registration);

    #PURCHASE DATE
    $purchasedDate = Get-RandomDate -minDate "2020-01-01" -maxDate "2023-05-31";
    #Write-Output $purchasedDate;

    #REGISTERED DATE 
    $registeredDate = Get-RandomDate -minDate "2020-01-01" -maxDate "2023-05-31";
    #Write-Output $registeredDate;

    #MANUFACTURED DATE
    $manufacturedDate = Get-RandomDate -minDate "2020-01-01" -maxDate "2023-05-31";
